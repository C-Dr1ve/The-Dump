...
local function RoflCopter()
	print("RoflCopter activated!") 
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if input.UserInputType == Enum.UserInputType.Keyboard then
		if input.KeyCode == Enum.KeyCode.Q then
			roflcopter = not roflcopter
			if roflcopter then
				pcall(RoflCopter)
				print("RoflCopter toggled ON")
			else
				print("RoflCopter toggled OFF")
			end
		end
	end
end)
...
local act = act or { key = {}, mousedown = false, keydown = false, Walking = false, Jumping = false, Anim = "" }
local roflcopter = false
local bodypos = bodypos or Instance.new("BodyPosition")
...
spawn(function()
	while true do
		if (act.key["w"] or act.key["a"] or act.key["s"] or act.key["d"]) and not roflcopter then
			if not act.Walking then act.Walking = true end
			if Anims and Anims.Walking then
				pcall(Anims.Walking)
			end
		else
			act.Walking = false
...
local function canPressKey(key)
	local last = lastKeyPress[key] or 0
	local cd = keyCooldowns[key] or 0
	if tick() - last >= cd then
		lastKeyPress[key] = tick()
		return true
	end
	return false
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if not act.key[keyName] and canPressKey(keyName) then
		act.key[keyName] = true
		act.keydown = true

		-- Actions
		if keyName == "q" then
			roflcopter = not roflcopter
			if roflcopter and RoflCopter then pcall(RoflCopter) end
		elseif keyName == "e" then
			if bodypos and bodypos.Position then
		...
		elseif keyName == "g" then
			if Shoot2 then pcall(Shoot2) end
		elseif keyName == "h" then
			if Shoot3 then pcall(Shoot3) end
		elseif keyName == "j" then
			if Shoot4 then pcall(Shoot4) end
		elseif keyName == "z" then
			if Attack then pcall(Attack) end
		elseif keyName == "x" then
			if DualAttack then pcall(DualAttack) end
		elseif keyName == "c" then
			if MegaBonk then pcall(MegaBonk) end
		elseif keyName == "v" then
			if Stomp then pcall(Stomp) end
		elseif keyName == " " then
...
UserInputService.InputEnded:Connect(function(input)
	if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
	local keyName = input.KeyCode.Name:lower()
	act.key[keyName] = false
	act.keydown = false
end)

mouse.Button1Down:Connect(function()
	if canPressKey("mouse1") then
		act.mousedown = true
	end
end)
